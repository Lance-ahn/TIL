...
/*
 ...
 타입을 만든 뒤 파싱 데이터를 통해 Astronaut 객체를 생성하여 출력
 */
print("\n---------- [ Practice ] ----------\n")
...
let dataTask = URLSession.shared.dataTask(with: apiURL) { (data, response, error) in
  ...
  // 코드 구현
    ...
    guard let people = jsonData["people"] as? [[String: String]] else { return }
    guard let peopleCount = jsonData["number"] as? Int else { return print("Parsing Error") }
...
//    }.forEach { print($0) }
    ...
    for i in people {
        if let craft = i["craft"], let name = i["name"] {
            print(Astronaut(craft: craft , name: name))
        }
    }
}
dataTask.resume()
